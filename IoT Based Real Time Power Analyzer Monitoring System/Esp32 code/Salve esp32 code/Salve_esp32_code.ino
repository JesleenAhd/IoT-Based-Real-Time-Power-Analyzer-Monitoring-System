...
// Replace with IP of master ESP32
const char* serverIP = "192.168.43.18";  // Change to actual IP of master ESP32
...
const int registerCount = sizeof(modbusRegisters) / sizeof(modbusRegisters[0]);
...
void loop() {
  ...
  for (int s = 0; s < NUM_SLAVES; s++) {
    float modbusValues[numParams];

    for (int i = 0; i < numParams; i++) {
      modbusValues[i] = readModbusFloat(SLAVE_IDS[s], modbusRegisters[i]);
      Serial.print("Slave ");
      Serial.print(SLAVE_IDS[s]);
      Serial.print(" - ");
      Serial.print(paramNames[i]);
      Serial.print(": ");
      Serial.println(modbusValues[i], 2);

      floatToRegisters(modbusValues[i], i * 2, s);
    }

    sendToMasterESP(modbusValues, SLAVE_IDS[s]);
    Serial.println("-------------------------------------------------");
  }
...
void sendToMasterESP(float modbusValues[], uint8_t slaveID) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    // Prepend the slave parameter to the URL
    String url = "http://" + String(serverIP) + "/send?slave=" + String(slaveID) + "&";

    // Append all parameters to the URL
    for (int i = 0; i < registerCount; i++) {
      url += urlEncode(String(paramNames[i])) + "=" + String(modbusValues[i], 2);
      if (i < registerCount - 1) url += "&";
    }

    Serial.println("Sending URL: " + url); // Debug print the URL

    http.begin(url);
    int httpResponseCode = http.GET();

    if (httpResponseCode > 0) {
      Serial.print("📨 Sent all data to Master. Response: ");
      Serial.println(http.getString());
    } else {
      Serial.print("❌ Failed to send HTTP GET: ");
  ...
  if (client && client.connected() && client.available() >= 12) {
    ...
    for (int i = 0; i < quantity; i++) {
      uint16_t val = holdingRegs[slaveIndex][startAddr + i];
            response[9 + i * 2] = val >> 8;           // High byte
      response[9 + i * 2 + 1] = val & 0xFF;     // Low byte
    }

    client.write(response, 9 + quantity * 2);
    Serial.println("✅ Sent Modbus TCP response to client");
  }
}
